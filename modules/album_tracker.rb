...
module AlbumTracker
  ...
  def store_album
    if File.exist?('albums.json')
      ...
      albums << {id: id, name: name, publish_date: publish_date, on_spotify: on_spotify, archived: archived, genre_id: genre.id}
      File.write('./albums.json', JSON.pretty_generate(albums))
    else
      File.write('./albums.json', JSON.pretty_generate([{id: id, name: name, publish_date: publish_date, on_spotify: on_spotify, archived: archived, genre_id: genre.id}]))
    end
  end
  ...
  def self.load_albums
    ...
    albums.each do |album|
      ...
      MusicAlbum.new(album['name'], album['publish_date'], album['id'], album['archived'], genre_object, album['on_spotify'])
    end
  end
end